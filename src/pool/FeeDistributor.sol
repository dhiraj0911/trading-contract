...
import {PoolTokenInfo} from "./PoolStorage.sol";

interface IPoolFeeDistributor is IPool {

    function poolTokens(address) external view returns (PoolTokenInfo memory);
    function withdrawFee(address, address) external;
}
...
contract FeeDistributor is Ownable, Pausable, ReentrancyGuard{
    ...
    IPoolFeeDistributor public immutable pool;
    address[] public feeTokens;

    EnumerableMap.AddressToUintMap private recipients;
    uint256 public totalWeight;

    constructor(address _pool) Ownable() Pausable() ReentrancyGuard() {
        if (_pool == address(0)) revert InvalidAddress();
        pool = IPoolFeeDistributor(_pool);
    }
    ...
    function _withdrawFee(address _token) internal returns (uint256 withdrawnAmount) {
        PoolTokenInfo memory tokenInfo = pool.poolTokens(_token);
        if (tokenInfo.feeReserve > 0) {
