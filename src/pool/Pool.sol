...
contract Pool is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PoolStorage {
        ...
        onlyAsset(_tokenOut)
    {
        ...
        (uint256 amountOutAfterFee, uint256 swapFee) = calcSwapOutput(_tokenIn, _tokenOut, amountIn);
        ...
        returns (uint256 amountInAfterFee, uint256 daoFee, uint256 lpAmount)
    {
        ...
        uint256 tokenPrice = _getPrice(_token, false, isStableCoin[_token]);
    ...
    {
        tokenPrice = _getPrice(_tokenOut, true, isStableCoin[_tokenOut]);
    ...
    {
        uint256 priceIn = _getPrice(_tokenIn, false, isStableCoin[_tokenIn]);
        uint256 priceOut = _getPrice(_tokenOut, true, isStableCoin[_tokenOut]);
    ...
    function _accrueInterest(address _token) internal returns (uint256) {
        ...
        AssetInfo memory asset = getPoolAsset(_token);
    ...
    function _calcFeeRate(      
        ...
        uint256 _currentValue = _tokenPrice * getPoolAsset(_token).poolAmount;
    ...
    function _getPoolValue(bool _max) internal view returns (uint256 sum) {
        ...
        for (uint256 i; i < allTranches.length;) {
    ...
    function getPoolAsset(address _token) public view returns (AssetInfo memory asset) {
        for (uint256 i; i < allTranches.length;) {
    ...
    function _reservePoolAsset(
        ...
        AssetInfo memory collateral = getPoolAsset(_collateralToken);
    ...
    function _releasePoolAsset(
        ...
        if (getPoolAsset(_collateralToken).reservedAmount < _vars.reserveReduced) {
    ...
    function _reserveTrancheAsset(
        ...
        for (uint256 i; i < shares.length;) {
            ...
            if (_side == Side.LONG) {
                ...
                // Asset used for long or short is same stable coin
                AssetInfo memory indexAsset = trancheAssets[tranche][_collateralToken];
                ...
                _validateGlobalShortSize(_collateralToken, indexAsset, indexPrice);
                trancheAssets[tranche][_collateralToken] = indexAsset;
            }
    ...
    function _releaseTranchesAsset(
        ...
        for (uint256 i; i < allTranches.length;) {
            ...
            uint256 lpFee =
                MathUtils.frac(_vars.totalLpFee, riskFactor[_indexToken][tranche], totalRiskFactor[_indexToken]);
            collateral.poolAmount = (
                (collateral.poolAmount + lpFee).toInt256() - frac(_vars.poolAmountReduced, share, totalShare)
            ).toUint256();

            int256 pnl = frac(_vars.pnl, share, totalShare);
            if (_side == Side.LONG) {
                collateral.guaranteedValue = MathUtils.addThenSubWithFraction(
    ...
    function _calcDecreasePayout(
        ...
        vars.collateralPrice = _getCollateralPrice(_collateralToken);
        ...
        if (_side == Side.LONG) {
            ...
            poolValueReduced = cap(poolValueReduced,
    ...
    function _getPrice(address _token, bool _max, bool isStableCoin) internal view returns (uint256) {
        if(isStableCoin) return 10 ** (USD_VALUE_DECIMAL - IERC20Metadata(_token).decimals());
