...
contract RwaxOracle is Ownable, IRwaxOracle {
    ...
    function getIndexPrice(address token, bool max) public view returns(uint256){
        (uint256 lastPrice, uint256 lastPriceTimestamp) = _getLastPrice(token);
        
        if (lastPriceTimestamp + PRICE_FEED_ERROR < block.timestamp) {
            return _getPriceSpread(lastPrice, PRICE_FEED_ERROR_SPREAD, max);
        }

        if (lastPriceTimestamp + PRICE_FEED_INACTIVE < block.timestamp) {
            return _getPriceSpread(lastPrice, PRICE_FEED_INACTIVE_SPREAD, max);
        }

        (uint256 lowerBound, uint256 minLowerBound, uint256 upperBound, uint256 maxUpperBound)= 
            _getIndexBounds(token, lastPrice);

        if (lastPrice > upperBound) {
            return max ? _min(lastPrice, maxUpperBound) : lastPrice;
        }

        if (lastPrice < lowerBound) {
            return max ? lastPrice : _max(lastPrice, minLowerBound);
        } 
    ...
    function _getPrice(address token, bool max) internal view returns (uint256) {
        ...
        /// newAdd
        /**
         * Not doing Reference Feed for now
    ...
    function _getIndexBounds(address token, uint256 price) 
        internal 
        view 
    ...
    {
        TokenConfig memory config = tokenConfig[token];
        lowerBound = price * (PRECISION - config.chainlinkDeviation) / PRECISION;
        minLowerBound = price * (PRECISION - 3 * config.chainlinkDeviation) / PRECISION;
        upperBound = price * (PRECISION + config.chainlinkDeviation) / PRECISION;
        maxUpperBound = price * (PRECISION + 3 * config.chainlinkDeviation) / PRECISION;
